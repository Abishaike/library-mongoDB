    ...
    */
    try {
      ...
      for (i = 0; i < selectBooksIDs.length; i++) {
        borrowArray.push(selectBooksIDs[i]);
      }
    } catch (err) {
      console.log(`err = ${err}`);
    }
    ...
    */
    try {
      user = await db.collection("clients").findOneAndUpdate
        ({ username: req.session.userName }, { $set: { IDBooksBorrowed: borrowArray } });
    } catch (err) {
      console.log(`err = ${err}`);
    }
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    /*
        Update all of the "borrowed books" to "available: false" within the "books" collection
    */
    try {
      for (i = 0; i < selectBooks.length; i++) {
        userBorrowed = await db.collection("books").findOneAndUpdate
          ({ title: selectBooks[i] }, { $set: { available: false } });
      }
    } catch (err) {
      console.log(`err = ${err}`);
    }
